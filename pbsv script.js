document.addEventListener('DOMContentLoaded', () => {
	...
	const updateUIWithPokemonData = (data) => {
		currentPokemonData = data; // store entire data object
		isShiny = false;
		...
		const defaultArtworkUrl = officialArtwork ? officialArtwork.front_default : null;
		const shinyArtworkUrl = officialArtwork ? officialArtwork.front_shiny : null;

		if (defaultArtworkUrl) {
			uploadedPortrait.src = defaultArtworkUrl;
			uploadedPortrait.style.display = 'block';
			portraitPlaceholder.classList.add('hidden');
			if (shinyArtworkUrl) {
			...
			portraitPlaceholder.classList.remove('hidden');
			shinyToggleButton.style.display = 'none';
			console.warn(`No official artwork found for ${data.name}.`);
		}
	};
	...
	shinyToggleButton.addEventListener('click', () => {
		if (currentPokemonData && currentPokemonData.sprites.other['official-artwork']) {
			const officialArtwork = currentPokemonData.sprites.other['official-artwork'];
			const defaultArtworkUrl = officialArtwork.front_default;
			const shinyArtworkUrl = officialArtwork.front_shiny;

			if (shinyArtworkUrl) {
				isShiny = !isShiny;
				uploadedPortrait.src = isShiny ? shinyArtworkUrl : defaultArtworkUrl;
			} else {
				console.warn("No shiny official artwork available for this Pokémon.");
				isShiny = false;
				uploadedPortrait.src = defaultArtworkUrl;
			}
		}

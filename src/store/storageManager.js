/**
 * storageManager.js - Centralized IndexedDB storage management
 ...
 */
export const set = async (storeName, key, value) => {
	if (!db) {
 ...
 */
export const clearExpired = async (storeName) => {
	if (!db) return;
 ...
 */
export const savePortrait = async (name, blobData) => {
	if (!db) {
 ...
 */
export const getPortraitUrl = async (name) => {
	if (!db) return null;
